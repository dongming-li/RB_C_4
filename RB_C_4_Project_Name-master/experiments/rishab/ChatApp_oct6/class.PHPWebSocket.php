...
class PHPWebSocket
{
	// constant which defines max number of developers. Need to set like atleast
    // 8 but was experiencing immense lag. Probably my local serveer hardware specification issue.
	const WS_MAX_CLIENTS = 6;

	// max client on an IP
	const WS_MAX_CLIENTS_PER_IP = 6;

	// amount of seconds the chattee has to send data to the server, before a ping request is sent to the chattee,
	// if the chattee has not completed the opening handshake, the ping request is skipped and the chattee connection is closed
	...
	// the maximum length, in bytes, of a message's payload data.
